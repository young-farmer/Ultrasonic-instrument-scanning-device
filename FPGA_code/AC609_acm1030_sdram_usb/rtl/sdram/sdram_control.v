...
`include        "Sdram_Params.h"
  ...
  output reg        Wdata_done;    //一次写突发完成标识位
  output reg        Rdata_done;    //一次读突发完成标识位

  output            Init_done;     //SDRAM初始化完成标志位
  ...
  reg             ref_break_wr;    //写操作过程中，刷新定时到来到此次写操作结束有效区间
  reg             ref_break_rd;    //读操作过程中，刷新定时到来到此次读操作结束有效区间
  reg             wr_break_ref;    //刷新过程中，写操作到来到此次刷新结束有效区间
  reg             rd_break_ref;    //刷新过程中，读操作到来到此次刷新结束有效区间
  ...
  begin
    if(!Rst_n)
      ref_time_cnt <= 'd0;
    else if(ref_time_cnt >= AUTO_REF - 1'b1)
      ref_time_cnt <= 'd0;
    else if(Init_done)
  ...
  localparam wr_ACT_TIME   = 1'b1,
             ...
             wr_BUST_TER   = (SDR_BL == 3'b111)?(SC_RCD+1+SC_BL):10'h3ff,
             wr_PRE_TIME   = SC_RCD+1+SC_BL+WR_PRE,
  ...
  localparam rd_ACT_TIME  = 1'b1,
             ...
             rd_BUST_TER  = (SDR_BL == 3'b111)?(SC_RCD+1+SC_CL+SC_BL-(SC_CL-1)-1):10'h3ff,
             rd_PRE_TIME  = (SDR_BL == 3'b111)?(SC_RCD+1+SC_CL+SC_BL-(SC_CL-1)):SC_RCD+1+SC_CL+SC_BL+1-REF_PRE,
             rd_END_TIME  = SC_RCD+1+SC_CL+SC_BL+1;
  ...
  begin
    if(!Rst_n)
      Rdata_done <= 1'b0;
    else if(rd_cnt == SC_RCD+SC_CL+SC_BL)
      Rdata_done <= 1'b1; 
    else
      Rdata_done <= 1'b0;
  end
  ...
  begin
    if(!Rst_n)
      ref_break_wr <= 1'b0;
    else if(ref_time_flag && wr_opt)
      ref_break_wr <= 1'b1;
    else if(!wr_opt)
      ref_break_wr <= 1'b0;
    else
      ref_break_wr <= ref_break_wr;
  end
...
//                        ((!wr_opt)?1'b0:ref_break_wr);
  ...
  begin
    if(!Rst_n)
      ref_break_rd <= 1'b0;
    else if(ref_time_flag && rd_opt)
      ref_break_rd <= 1'b1;
    else if(!rd_opt)
      ref_break_rd <= 1'b0;
    else
      ref_break_rd <= ref_break_rd;
  end
...
//                        ((!ref_opt)?1'b0:rd_break_ref));
  ...
  begin
    case(main_state)
      AREF:begin
        if(!ref_opt && (!ref_time_flag)&& !Wr && Rd )
          rd_req = 1'b1;
